...
export function Comments({ itemId }: CommentsProps) {
  ...
  const handleEditComment = async (commentId: Id<"comments">) => {
    // Double-check authentication and ownership
...
function CommentItem({
  ...
  // CRITICAL: Must be authenticated AND logged in user must exist AND be the owner
  // loggedInUser._id is the user ID, loggedInUser.profile contains the profile info
  const isOwner = isAuthenticated && loggedInUser && loggedInUser._id === comment.userId;
  const canEdit = isOwner; // Only authenticated owner can edit
  const canDelete = isOwner; // Only authenticated owner can delete

  // Debug logging (remove in production)
  if (process.env.NODE_ENV === 'development') {
    console.log('Comment permissions check:', {
      isAuthenticated,
