...
export function Director() {
  ...
  const director = useQuery(api.directors.getDirectorBySlug, { slug: slug || "" });
  const filmography = useQuery(
    api.directors.getDirectorFilmography, 
    director ? { directorName: director.name } : "skip"
  );
  ...
  // Show not found state - either no slug provided or director not found
  if (!slug || director === null) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-gray-900 mb-4">
            {!slug ? "No Director Selected" : "Director Not Found"}
          </h2>
          <p className="text-gray-600 mb-4">
            {!slug 
              ? "Please select a director to view their profile." 
  ...
  // Only use dynamic director data from database
      ...
      <section className="section section--details">
        ...
        <div className="container">
          <div className="row">
            {/* Title */}
            <div className="col-12">
              <h1 className="section__title section__title--head">{directorData.name}</h1>
            </div>
                ...
                <div className="row">
                  <div className="col-12 col-sm-5 col-md-5">
                    <div className="item__cover">
                      <img src={directorData.imageUrl || "/img/covers/actor.jpg"} alt={directorData.name} />
                    </div>
                  </div>
                  ...
                  <div className="col-12 col-md-7">
                    <div className="item__content">
                      <ul className="item__meta">
                        <li><span>Career:</span> {directorData.career}</li>
                        {directorData.height && <li><span>Height:</span> {directorData.height}</li>}
                        {directorData.dateOfBirth && <li><span>Date of birth:</span> {directorData.dateOfBirth}</li>}
                        {directorData.placeOfBirth && <li><span>Place of birth:</span> {directorData.placeOfBirth}</li>}
                        {directorData.age && <li><span>Age:</span> {directorData.age}</li>}
                        {directorData.zodiac && <li><span>Zodiac:</span> {directorData.zodiac}</li>}
                        {directorData.genres && directorData.genres.length > 0 && (
                          <li>
                            <span>Genres:</span> 
                            {directorData.genres.map((genre, index) => (
                              <a key={index} href="#" className="mr-2">{genre}</a>
                            ))}
                          </li>
                        )}
                        {directorData.totalMovies && <li><span>Total number of movies:</span> {directorData.totalMovies}</li>}
                        {directorData.firstMovie && <li><span>First movie:</span> <a href="/details">{directorData.firstMovie}</a></li>}
                        {directorData.lastMovie && <li><span>Last movie:</span> <a href="/details">{directorData.lastMovie}</a></li>}
                        {directorData.bestMovie && <li><span>Best movie:</span> <a href="/details">{directorData.bestMovie}</a></li>}
                      </ul>
      ...
      <div className="row">
        <div className="col-12">
          <div className="text-center py-5">
            <p className="text-gray-500">No filmography available for this director.</p>
          </div>
        </div>
      </div>
    );
